...
There's no need to add a new Resources folder for this demo since an existing folder one level up is used.
...
> **Note**: The agent reasoning activity and all tool activities use `DaprConversationClient` with constructor injection:
> ```csharp
...
> }
> ```
> The agent reasoning uses LLM to make autonomous decisions with `ConversationOptions("conversation")` (no model specification), while tool activities may use LLM for simulation or use deterministic logic.
...
apps:
  - appID: space-debris-cleanup-app
    appDirPath: /
    appPort: 5500
    daprHTTPPort: 3550
    command: ["dotnet", "run"]
...
### Step 8: DemoTime file
**6-autonomousagent-spacedebris.yaml**

Create a new DemoTime yaml file in the .demo folder to start and stop the `space-debris-cleanup-app` app using the Dapr CLI. Use existing DemoTime yaml files in the .demo folder as a reference.

### Step 9: Local HTTP Tests
**local.http**
```http
### Start a LEO cleanup mission
# Note: String values for enums (orbitalZone) are automatically converted
...
### Start an aggressive mission with MEO orbit
POST http://localhost:5500/mission/start
Content-Type: application/json

{
  "missionId": "CLEANUP-MEO-001",
  "orbitalZone": "MEO",
  "maxDebrisPieces": 10,
  "fuelBudget": 50.0,
  "maxMissionHours": 4,
...
### Send human approval (approve)
POST http://localhost:5500/mission/mission-CLEANUP-LEO-001/approval
Content-Type: application/json

{
  "approved": true,
  "reason": "Mission Control approves high-risk maneuver"
}

### Send human approval (disapprove)
POST http://localhost:5500/mission/mission-CLEANUP-LEO-001/approval
Content-Type: application/json

{
  "approved": false,
  "reason": "Too risky - abort maneuver and return to safe distance"
}
```
